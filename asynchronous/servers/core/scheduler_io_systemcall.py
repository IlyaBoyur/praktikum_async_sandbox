...
from .system_calls import SystemCall
...
class Scheduler:
    ...
    def wait_for_read(self, task: Task, fd: int):
        try:
            ...
            mask, (_, writer) = key.events, key.data
    ...
    def wait_for_write(self, task: Task, fd: int):
        try:
            ...
            mask, (reader, _) = key.events, key.data
            self.selector.modify(fd, mask | EVENT_WRITE, (reader, task))

    def _remove_reader(self, fileobj):
        try:
            ...
            if not mask:
                ...
                self.selector.modify(fileobj, mask, (None, writer))

    def _remove_writer(self, fileobj):
        try:
            key = self.selector.get_key(fileobj)
        except KeyError:
            pass
            ...
            if not mask:
                ...
                self.selector.modify(fileobj, mask, (reader, None))

    def io_poll(self, timeout: None | float):
